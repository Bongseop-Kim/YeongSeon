...
- Fal CI 패턴 파이프라인을 켤 때는 서버 `FALAI_CI_PATTERN_ENABLED` 와 클라이언트 `VITE_FALAI_CI_PATTERN_ENABLED` 를 항상 함께 토글합니다.
- 런타임 상태 확인은 `generate-fal-api` 의 `GET /should-use-fal-pipeline` 엔드포인트를 사용합니다. 응답은 `enabled`, `serverVar`, `clientVar` 를 반환합니다.
