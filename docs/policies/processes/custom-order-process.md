...
| `접수`                      | 클레임(cancel) 생성 후 관리자 처리. 전액 환불 (샘플 미시작)                                             |
| `샘플원단제작중`~`샘플승인` | 클레임(cancel) 생성 후 관리자 처리. `sample_cost` 공제 후 환불                                          |
| `제작중`                    | 클레임(cancel) 생성 후 관리자 처리. 샘플 있는 경우 `sample_cost` 공제 후 환불, 샘플 없는 경우 전액 환불 |
