...
| Ask first | `발송대기`/`발송중` 미구현 상태 활성화. 취소 환불 비율 조정.                                                                              |
...
1. **BR-repair-001**: 결제 완료 시 repair 주문은 `결제중` → `접수`로 직접 전이 (`발송대기`/`발송중` 미구현)
2. **BR-repair-002**: Toss 결제 실패 시 `결제중` → `대기중` 자동 복구
3. **BR-repair-003**: 롤백 가능 상태: `접수` → `대기중`, `수선중` → `접수`, `수선완료` → `수선중`. memo 필수
4. **BR-repair-004**: `배송중`/`배송완료`/`완료`/`취소`는 롤백 불가
5. **BR-repair-005**: `접수` 이후(`접수`, `수선중`, `수선완료`, `배송중`, `배송완료`, `완료`)에서 취소 불가
6. **BR-repair-006**: 취소 환불 규칙 — `대기중`/`발송대기`: 전액 환불. `발송중`: 반품 택배비(`REFORM_SHIPPING_COST`) 공제 후 환불 **[미구현]**
