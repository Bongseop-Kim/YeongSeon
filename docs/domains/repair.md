...
수선 주문은 배송이 두 방향으로 발생한다.

| 방향          | DB 컬럼                                                                    | 입력 주체                                  | 발생 시점                     |
| ------------- | -------------------------------------------------------------------------- | ------------------------------------------ | ----------------------------- |
| **고객→회사** | `courier_company`, `tracking_number`, `shipped_at`                         | 고객 (`submit_repair_tracking` RPC)        | `발송대기` → `발송중` 전환 시 |
...
| `배송중`   | `완료`     | 구매확정 (수동) 또는 company_shipped_at 기준 7일 경과 (자동) |
