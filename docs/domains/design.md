...
고객이 텍스트 또는 이미지를 입력하면 AI 모델이 넥타이 디자인을 생성하는 단발성 프로세스. 별도 상태 전이 없이 요청→생성→완료(또는 실패)로 즉시 종료. 생성 비용은 디자인 토큰으로 차감되며 실패 시 자동 환불.

## 경계

| 구분      | 내용                                                                                                                                      |
| --------- | ----------------------------------------------------------------------------------------------------------------------------------------- |
| Always do | 토큰 차감 전 잔액 확인. 환불 대기(pending) 중 생성 요청 차단. paid 토큰 먼저 차감 후 bonus 차감. work_id 기반 멱등 환불로 중복 환불 방지. |
...
- **BR-design-004**: 이미지 미생성 시(데이터 부족 또는 텍스트 전용 응답) `refund_design_tokens`로 자동 환불. `work_id` 기반 멱등 처리.
