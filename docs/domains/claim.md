...
| Never do  | 수거완료/재발송/완료 상태에서 롤백. quote-request/design-token에 클레임 생성. 현재 서버가 허용하지 않는 주문 상태에서 클레임 생성 가정. 활성 클레임 존재 중 주문 상태를 admin에서 직접 변경. |
...
- **BR-claim-011**: 주문당 활성 클레임은 최대 1건이다. 활성 상태는 `접수`, `처리중`, `수거요청`, `수거완료`, `재발송`이다.
- **BR-claim-012**: 부분 수량 클레임이어도 주문 전체는 단일 클레임 흐름에 들어간 것으로 본다.
- **BR-claim-013**: 활성 클레임이 존재하는 동안 admin은 주문 상태를 직접 변경할 수 없다. 주문 상태 전이는 클레임 처리 흐름에서 진행한다.
