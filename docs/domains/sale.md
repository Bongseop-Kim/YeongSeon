...
| 구분      | 규칙                                                                                                                               |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Always do | 모든 상태 전이에 `auth.uid()` 소유권 검증 포함. 결제 흐름은 Edge Function을 통해서만 처리. 구매확정 전 진행 중인 클레임 여부 확인. |
...
6. **BR-sale-006**: 구매확정(수동): 고객이 직접 `customer_confirm_purchase` 호출. 허용 상태: `배송완료`, `배송중`. 진행 중인 클레임 없어야 함
