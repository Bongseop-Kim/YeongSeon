...
> 주문제작은 샘플 주문과 분리된 독립 주문 도메인이다. store `/custom-order`에서 생성되며 `order_type='custom'`만 사용한다.
...
2. 환불은 `대기중/결제중/접수`에서만 전액 환불 가능하다.
3. `orders.sample_cost`는 custom 주문에서 항상 0으로 유지한다.
4. 샘플 제작은 [[sample]] 도메인에서만 처리한다.
