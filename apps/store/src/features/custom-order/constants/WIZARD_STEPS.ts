...
export const SAMPLE_WIZARD_STEPS: StepConfig[] = [
  {
    id: "sample-setup",
    label: "샘플 설정",
    validate: (values) => {
  ...
  },
  {
    ...
    label: "원단",
    validate: (values) => {
      if (values.fabricProvided) return null;
      if (!values.fabricType) return "원단 소재를 선택해주세요";
      if (!values.designType) return "디자인 방식을 선택해주세요";
      return null;
    },
    isSkippable: (values) => values.fabricProvided,
  },
  {
    id: "sewing",
    label: "봉제",
    ...
    isSkippable: (values) => values.sampleType === "fabric",
  },
  {
    id: "spec",
    label: "규격",
