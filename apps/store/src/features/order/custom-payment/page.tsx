    ...
    try {
      ...
      if (!orderId) {
        if (state.orderType === "custom") {
          ...
          orderId = (await createCustomOrder.mutateAsync(request)).orderId;
        } else {
          orderId = (
            await createSampleOrder.mutateAsync({
              shippingAddressId: selectedAddressId,
      ...
      await paymentWidgetRef.current.requestPayment({
        ...
        orderName:
          state.orderType === "custom"
            ? `주문제작 (수량 ${state.coreOptions.quantity}개)`
            : "샘플 주문",
      ...
      ? [
          {
            id: "amount",
            label: "상품 금액",
            value: `${state.totalCost.toLocaleString()}원`,
          },
          {
            id: "quantity",
            label: "수량",
      ...
      : [
          {
            id: "amount",
            label: "상품 금액",
            value: `${state.samplePrice.toLocaleString()}원`,
          },
        ];
        ...
        <MainContent className="overflow-visible">
          ...
          >
            <div className="space-y-8">
              <UtilityPageIntro
                eyebrow={
                  state.orderType === "custom" ? "Custom Order" : "Sample Order"
                }
                title={
                  state.orderType === "custom"
                    ? "주문제작 결제"
                ...
                meta={
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-zinc-600">
                    <span>
                      {state.orderType === "custom" ? "주문제작" : "샘플 주문"}{" "}
              ...
              >
                <div className="divide-y divide-zinc-100 border-y border-stone-200 py-2 text-sm">
                  {state.orderType === "custom" ? (
                    <>
                      ...
                      <div className="flex items-center justify-between py-3">
                        <span className="text-zinc-500">상세 옵션</span>
                        <span className="text-zinc-950">
                    ...
                    <>
                      ...
                      {state.options.fabricType && (
                        <div className="flex items-center justify-between py-3">
                          <span className="text-zinc-500">원단 조합</span>
                          <span className="text-zinc-950">
                            {state.fabricLabel}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center justify-between py-3">
                        <span className="text-zinc-500">타이 방식</span>
                        <span className="text-zinc-950">
                          {getTieTypeLabel(state.options.tieType)}
                        </span>
                      </div>
                      <div className="flex items-center justify-between py-3">
                        <span className="text-zinc-500">심지</span>
                        <span className="text-zinc-950">
                  ...
                  <div className="py-3">
                    <span className="text-zinc-500">참고 이미지</span>
                    {state.imageRefs.length > 0 ? (
                      ...
                      <span className="ml-4 text-zinc-950">없음</span>
                    )}
                  </div>
                  {state.additionalNotes ? (
                    <div className="py-3">
                      <span className="text-zinc-500">요청사항</span>
                      <p className="mt-2 whitespace-pre-wrap text-zinc-950">
                        {state.additionalNotes}
                      </p>
                    </div>
                  ) : null}
