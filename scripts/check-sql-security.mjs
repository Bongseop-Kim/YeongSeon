...
for (const file of sqlFiles) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    const nameMatch = part.match(/\bCREATE\s+(?:OR\s+REPLACE\s+)?FUNCTION\s+([^\s(]+)/i)
    ...
    violations.push(`  ${file.replace(ROOT + '/', '')}: ${funcName}`)
  }
}
...
process.stdout.write(`✅ SECURITY DEFINER 검사 통과 (${sqlFiles.length}개 파일)\n`)
