-- =============================================================
-- 20260315000000_fix_code_review_issues.sql
-- PR 코드 리뷰 이슈 수정:
--   1. calculate_custom_order_amounts: fabric_amount 에러 발생 (4-6)
--   2. confirm_payment_orders: design_tokens work_id + 멱등성 (4-11)
--   3. REVOKE confirm_payment_orders from anon/authenticated (4-7)
--   4. create_claim: public 접두사, FOR UPDATE, token 취소 분기 (4-8, 4-9)
--   5. admin_update_order_status: token 완료 시 payment_key 검증 (4-10)
-- =============================================================

-- ── 1. calculate_custom_order_amounts: fabric_amount 에러 발생 ──
-- fabric_provided=false이지만 design_type/fabric_type 누락 시 0 반환 대신 EXCEPTION 발생
...
-- ── 2. confirm_payment_orders: design_tokens work_id + 멱등성 강화 ──
...
-- ── 3. confirm_payment_orders: service_role 전용 권한 설정 ──
...
-- ── 4. create_claim: public 접두사 정규화 + FOR UPDATE + token 취소 분기 ──
...
-- ── 5. admin_update_order_status: token 완료 시 payment_key 검증 ──
