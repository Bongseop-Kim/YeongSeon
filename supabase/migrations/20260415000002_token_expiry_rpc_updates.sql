-- =====================================================================
-- Token expiry RPC updates
-- Applies token expiry-aware logic to payment, balance, usage, refund,
-- and admin token management flows.
-- =====================================================================
...
BEGIN
  ...
  LOOP
    v_paid_bal := v_paid_bal + v_batch_row.remaining;
  END LOOP;

  SELECT COALESCE(SUM(amount), 0)::integer INTO v_legacy_paid_bal
  FROM public.design_tokens
  WHERE user_id = p_user_id AND token_class = 'paid' AND source_order_id IS NULL;
  v_paid_bal := v_paid_bal + v_legacy_paid_bal;
