-- SECURITY DEFINER 사용 근거 주석 추가
-- use_design_tokens, request_token_refund, approve_token_refund 함수에
-- 왜 SECURITY DEFINER를 사용하는지 명시하고, 내부 권한 검증 로직 존재를 주석으로 기록한다.

-- ── use_design_tokens ─────────────────────────────────────────
-- SECURITY INVOKER가 기본이며, 이 RPC는 authenticated 역할에 INSERT 권한이 없는
-- design_tokens 테이블에 차감 레코드를 직접 기록해야 하므로 SECURITY DEFINER를 사용한다.
-- 함수 내부에서 service_role JWT claim 또는 auth.uid() = p_user_id 비교를 통해
-- 호출자 소유권을 반드시 검증한다 (line: v_caller_role 또는 auth.uid() IS DISTINCT FROM p_user_id 체크).
...
-- SECURITY INVOKER가 기본이며, 이 RPC는 authenticated 역할에 INSERT 권한이 없는
-- token_refund_requests 테이블에 서버 측 검증된 금액으로 환불 요청을 기록해야 하므로
-- SECURITY DEFINER를 사용한다. 클라이언트가 환불 금액을 임의로 조작하지 못하도록
-- 서버에서 직접 계산·삽입하는 것이 핵심 목적이다.
-- 함수 내부에서 auth.uid() 로그인 여부 확인 및 v_order.user_id = v_user_id 비교를 통해
-- 호출자 소유권을 반드시 검증한다.
...
-- SECURITY INVOKER가 기본이며, 이 RPC는 service_role 전용 관리자 기능으로
-- design_tokens, orders, order_status_logs, token_refund_requests 등
-- 여러 테이블에 대한 쓰기 권한을 한 트랜잭션 내에서 수행해야 하므로 SECURITY DEFINER를 사용한다.
-- 함수 내부에서 JWT claims의 role = 'service_role' 여부를 검증하여
-- 일반 사용자(authenticated/anon)의 호출을 원천 차단한다.
-- REVOKE/GRANT를 통해 anon, authenticated 역할의 실행 권한도 명시적으로 제거되어 있다.
