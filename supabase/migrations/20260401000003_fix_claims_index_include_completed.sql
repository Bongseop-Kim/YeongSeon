-- fix: idx_claims_active_per_item에 '완료' 상태 추가 및 create_claim ON CONFLICT 정합성 보정
--
-- 문제: create_claim의 pre-check(step 7)는 '완료' 포함 중복 검사를 수행하지만,
--       ON CONFLICT에서 참조하는 부분 인덱스 idx_claims_active_per_item에는 '완료'가 빠져 있어
--       pre-check 이후 INSERT 시점에 race condition으로 완료 상태 클레임이 있어도 INSERT가 통과될 수 있음.
...
-- 2. create_claim: ON CONFLICT WHERE에 '완료' 추가
...
begin
  ...
  -- ON CONFLICT WHERE에 '완료' 포함: pre-check와 인덱스 조건 일치로 race condition 방지
