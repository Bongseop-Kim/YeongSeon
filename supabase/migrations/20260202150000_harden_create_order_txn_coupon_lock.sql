drop function if exists public.create_order_txn(
  uuid,
  jsonb,
  integer,
  integer,
  ...
  loop
    ...
    if v_applied_coupon_id is not null then
      ...
      if v_coupon.max_discount_amount is not null then
        v_discount_amount :=
          least(v_discount_amount, floor(v_coupon.max_discount_amount::numeric)::integer);
      end if;
    ...
    v_total_discount := v_total_discount + (v_discount_amount * v_quantity);
...
revoke all on function public.create_order_txn(uuid, jsonb) from public;
revoke all on function public.create_order_txn(uuid, jsonb) from anon;
grant execute on function public.create_order_txn(uuid, jsonb) to authenticated;
grant execute on function public.create_order_txn(uuid, jsonb) to service_role;
