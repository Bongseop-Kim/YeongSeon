-- 20260312000000_fix_create_order_txn_row_count.sql
-- create_order_txn: images UPDATE 후 v_row_count 다중 행 검사 + 에러 메시지 보안 강화 + SECURITY DEFINER 주석 추가

-- SECURITY DEFINER: RLS를 우회하여 orders, order_items, images, user_coupons,
-- products, product_options 등 여러 테이블에 트랜잭션 쓰기를 수행해야 하기 때문.
-- auth.uid()로 소유권 검증 후 실행되므로 권한 상승 위험 없음.
    ...
    loop
      ...
      IF v_tie_image IS NOT NULL AND v_tie_file_id IS NOT NULL THEN
        ...
        SET folder = '/reform',
            ...
            expires_at = NULL,
            url = v_tie_image
        WHERE file_id = v_tie_file_id
          AND uploaded_by = v_user_id
          AND deleted_at IS NULL;

        GET DIAGNOSTICS v_row_count = ROW_COUNT;
        IF v_row_count = 0 THEN
          RAISE EXCEPTION 'Image record not found for the provided tie image';
        ELSIF v_row_count > 1 THEN
          RAISE LOG 'Multiple image records found: % rows for file_id: %, user: %', v_row_count, v_tie_file_id, v_user_id;
          RAISE EXCEPTION 'Multiple image records found for the provided tie image';
        END IF;
      END IF;
    end loop;
