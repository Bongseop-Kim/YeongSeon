-- advisory lock 순서 수정 및 SECURITY DEFINER 주석 보강
--
-- 수정 사항:
-- 1. manage_design_tokens_admin: v_balance SELECT를 advisory lock 획득 후로 이동 (race condition 수정)
-- 2. manage_design_tokens_admin: SECURITY DEFINER 근거 주석 추가
-- 3. request_token_refund: SECURITY DEFINER 근거 주석 추가
...
-- 목적으로만 제한되며, 호출 전 is_admin() 검사로 진입 권한을 보장한다.
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE
