-- =============================================================
-- 20260331000001_fix_db_lint_followup.sql
-- 20260331000000 적용 후 남은 lint 오류 수정
--
-- 1. create_custom_order_txn: squash에 남아 있는 레거시 오버로드
--    (jsonb, integer, boolean, text, jsonb, uuid) 제거
-- 2. admin_update_order_status: ON CONFLICT partial index 조건 누락 수정
-- =============================================================

-- ── 1. create_custom_order_txn 레거시 오버로드 제거 ────────────
-- squash가 생성한 가장 오래된 오버로드로 파라미터 순서가 다름.
-- 이 버전이 order_items.reform_data 컬럼을 참조해 lint error 유발.
DROP FUNCTION IF EXISTS public.create_custom_order_txn(jsonb, integer, boolean, text, jsonb, uuid);

-- ── 2. admin_update_order_status ─────────────────────────────
-- design_tokens의 partial unique index는 WHERE work_id IS NOT NULL 조건이 있으므로
-- ON CONFLICT 절에도 동일한 predicate가 필요.
