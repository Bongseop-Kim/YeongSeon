-- =============================================================
-- 결제 사전 락(Pre-approval Lock) 패턴 도입
-- Toss 호출 전 대기중 → 결제중 원자적 전환으로 결제 진행 중 상태 명시화
-- =============================================================

-- 1. CHECK constraint에 '결제중' 추가
...
-- 2. lock_payment_orders: 대기중 → 결제중 원자적 전환
...
-- 3. unlock_payment_orders: 결제중 → 대기중 복구 (Toss 실패 시)
-- SECURITY DEFINER: order_status_logs INSERT에 일반 유저 RLS 없음
...
-- 4. confirm_payment_orders: 상태 검증 변경 — 결제중만 허용 (lock → confirm 순서 강제)
...
begin
  ...
  loop
    ...
    -- 반드시 lock_payment_orders로 결제중 상태여야 함
    if v_order.status != '결제중' then
      raise exception 'Order % is not payable (status: %)', v_order.id, v_order.status;
    end if;
...
-- 5. admin_update_order_status: 취소/롤백 전이에 결제중 추가
