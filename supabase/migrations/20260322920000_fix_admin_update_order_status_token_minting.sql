-- admin_update_order_status: 토큰 민팅 멱등성 강화 + SECURITY DEFINER 주석 추가
--
-- 수정 사항:
-- 1. SECURITY DEFINER 사용 근거 주석 추가
-- 2. 토큰 민팅 존재 체크 시 레거시 'order_<id>_paid' work_id 포맷도 함께 확인
--    (v_token_amount/v_plan_key SELECT를 EXISTS 체크 전으로 이동해 NULL INSERT 방지)
-- 3. ON CONFLICT 절을 부분 인덱스(WHERE work_id IS NOT NULL)와 일치하도록 수정
...
RETURNS jsonb
LANGUAGE plpgsql
-- SECURITY DEFINER 사유: 이 RPC는 관리자 전용 쓰기 기능이다.
-- SECURITY INVOKER로는 관리자가 다른 사용자 소유의 orders, design_tokens 레코드를
-- 수정·삽입할 수 없어 RLS 우회가 불가피하다.
-- 함수 내부에서 auth.uid() 및 public.is_admin()으로 호출자 권한을 검증한다.
SECURITY DEFINER
...
begin
  ...
  if p_new_status = '배송중' then
    ...
    if v_order_type = 'token' then
      ...
      -- 레거시 'order_<id>_paid' 포맷도 함께 체크해 중복 민팅 방지
      if not exists (
    ...
    if v_order_type = 'token' then
      -- ON CONFLICT: 부분 인덱스(WHERE work_id IS NOT NULL)와 일치시켜 런타임 오류 방지
