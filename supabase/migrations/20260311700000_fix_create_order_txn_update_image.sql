-- 20260311700000_fix_create_order_txn_update_image.sql
-- create_order_txn: INSERT → UPDATE 수정 (기존 임시 images 행을 업데이트)
  ...
  if jsonb_array_length(v_reform_items) > 0 then
    ...
    loop
      ...
      IF v_tie_image IS NOT NULL AND v_tie_file_id IS NOT NULL THEN
        ...
        IF NOT FOUND THEN
          RAISE EXCEPTION 'Image record not found for file_id: %', v_tie_file_id;
        END IF;
      END IF;
    end loop;
