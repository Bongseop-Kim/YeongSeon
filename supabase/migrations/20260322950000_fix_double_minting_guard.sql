-- admin_update_order_status 이중 민팅 방지 보강
--
-- 수정 사항:
-- 1. 완료 처리 시 토큰 주문 메타데이터를 항상 먼저 조회해 NULL INSERT 가능성을 제거
-- 2. order_<id>, order_<id>_paid 두 포맷을 모두 확인해 레거시 데이터까지 포함한 이중 민팅 방지
-- 3. 이미 민팅된 주문은 design_tokens INSERT를 건너뛰되, 상태 전이/로그 로직은 그대로 유지
