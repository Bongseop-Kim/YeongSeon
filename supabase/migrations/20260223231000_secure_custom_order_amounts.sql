...
begin
  ...
  return public.create_custom_order_txn(
    p_shipping_address_id => p_shipping_address_id,
    p_options => p_options,
    p_sewing_cost => v_sewing_cost,
    p_fabric_cost => v_fabric_cost,
    p_total_cost => v_total_cost,
    p_quantity => p_quantity,
    p_reference_image_urls => coalesce(p_reference_image_urls, '{}'::text[]),
    p_additional_notes => coalesce(p_additional_notes, ''),
    p_sample => coalesce(p_sample, false)
  );
end;
$function$;
