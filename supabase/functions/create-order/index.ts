...
Deno.serve(async (req) => {
  ...
  if (productError) {
    return jsonResponse(500, { error: "Failed to load products" });
  }
  ...
  if (optionError) {
    return jsonResponse(500, { error: "Failed to load product options" });
  }

  const optionPriceByKey = new Map<string, number>();
  ...
  const appliedCouponIds = Array.from(
    new Set(
      payload.items
        .map((item) => item.applied_user_coupon_id)
        .filter(Boolean)
    )
  ) as string[];
  ...
  if (couponError) {
    return jsonResponse(500, { error: "Failed to load coupons" });
  }

  const couponById = new Map<string, CouponRecord>();
  (coupons ?? []).forEach((coupon) => {
    couponById.set(coupon.id, coupon as CouponRecord);
  });
    ...
    orderItemsPayload = payload.items.map((item) => {
      ...
      return {
        ...
        reform_data: item.reform_data
          ? { ...item.reform_data, cost: REFORM_BASE_COST }
          : null,
  ...
  if (itemsError) {
    ...
    return jsonResponse(500, { error: "Failed to create order items" });
  }
  ...
  if (appliedCouponIds.length > 0) {
    ...
    if (couponUpdateError) {
      await supabase.from("order_items").delete().eq("order_id", orderId);
      await supabase.from("orders").delete().eq("id", orderId);
      return jsonResponse(500, { error: "Failed to update coupons" });
    }
  }
