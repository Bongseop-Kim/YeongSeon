    ...
    try {
      ...
      if (
        ...
        if (payload.previousImageBase64) {
          ...
          formData.append("prompt", prompts.imageEditPrompt);
          ...
          if (!primaryBase64) {
            throw new Error("Source image is required for image edit mode");
          }
          ...
          formData.append("prompt", prompts.imagePrompt);
          formData.append("model", "gpt-image-1.5");
          formData.append("size", "1024x1536");
          formData.append("quality", quality);
          if (payload.ciImageBase64 && payload.referenceImageBase64) {
...
const runOpenAiAnalysis = async (params: {
  adminClient: SupabaseClient;
  userId: string;
  payload: GenerateDesignRequest;
  conversationTurns: ConversationTurn[];
  ...
  try {
    ...
    const textResult = await requestOpenAIText(
      payload,
      openaiApiKey,
      conversationTurns,
      textPrompt,
    ...
    await emitAnalysisLog({
      normalizedDesign: normalizedDesign as Record<string, unknown>,
...
const runOpenAiRenderFromAnalysis = async (params: {
  adminClient: SupabaseClient;
  userId: string;
  payload: GenerateDesignRequest;
  ...
  const { adminClient, userId, payload, analysis, openaiApiKey } = params;
  ...
  if (rawImageUrl === null) {
    ...
    await emitRenderLog({
      ...
      errorMessage: "OpenAI image API failed",
    });
