...
**DALL·E 2 (`dall-e-2`) 마스크 편집 요구사항** (공식 문서 기준):

- **정사각형 PNG**
- **4MB 미만**
- **alpha channel 포함** (투명 영역 = 편집 대상)
- 마스킹 방식: 기계적인 영역 교체 (마스크로 지정한 부분을 프롬프트 지시로 채움)

**GPT Image (`gpt-image-1`, `gpt-image-1-mini`, `gpt-image-1.5`) 마스크 편집**:

- 이미지/마스크 형식·크기 제약은 DALL·E 2와 다를 수 있음 (검증 필요)
- 편집 방식: **프롬프트 유도형 편집** — DALL·E 2처럼 기계적으로 딱 잘라 바꾸는 방식이 아닌, 프롬프트로 변경 의도를 전달하는 방식
- 여러 이미지를 넣으면 **마스크는 첫 번째 이미지에 적용**됨 (검증 필요)

즉, GPT Image에서 위치 정밀도가 중요한 작업은 마스크만 믿기보다
